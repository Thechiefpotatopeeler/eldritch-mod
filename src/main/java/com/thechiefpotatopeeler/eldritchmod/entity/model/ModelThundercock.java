package com.thechiefpotatopeeler.eldritchmod.entity.model;// Made with Blockbench 4.7.4
...
public class ModelThundercock extends ModelBase {
	...
	public void setRotationAngles(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor, Entity entityIn){
		this.headWhole.rotateAngleX = headPitch * 0.017453292F;
		this.headWhole.rotateAngleY = netHeadYaw * 0.017453292F;
		copyModelAngles(this.headWhole, this.beak);
		copyModelAngles(this.headWhole, this.adamsApple);
		this.crest.rotateAngleX = headWhole.rotateAngleX;
		this.body.rotateAngleX = ((float)Math.PI / 2F);
		this.rightLeg.rotateAngleX = MathHelper.cos(limbSwing * 0.6662F) * 1.4F * limbSwingAmount;
		this.leftLeg.rotateAngleX = MathHelper.cos(limbSwing * 0.6662F + (float)Math.PI) * 1.4F * limbSwingAmount;
		this.rightWing.rotateAngleZ = MathHelper.cos(limbSwing * 0.4445F) * 0.7F * limbSwingAmount;
		this.leftWing.rotateAngleZ = -MathHelper.cos(limbSwing * 0.4445F) * 0.7F * limbSwingAmount;
	}
}